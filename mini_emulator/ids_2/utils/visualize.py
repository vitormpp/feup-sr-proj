...
PCAP file, saving individual matplotlib PNG plots to an output directory.

Usage
-----
    python visualize_models.py capture.pcap
    python visualize_models.py capture.pcap --algos rf lr svm --outliers iforest lof
    python visualize_models.py capture.pcap --test-size 0.3 --window 2.0
    python visualize_models.py capture.pcap --known 10.0.0.1 10.0.0.2
    python visualize_models.py capture.pcap --out-dir my_plots/
...
_DEFAULT_ALGOS = [k for k in CLASSIFIERS if k != "knn"]
...
def plot_summary_table(all_results: list[dict]) -> plt.Figure:
    ...
    for r in sorted_results:
        ...
        for m in metrics:
            ...
            row.append(f"{v:.4f}")
            if v == best[m]:
                row_colors.append("#1A3A20")  # green tint
            elif v == worst[m]:
                row_colors.append("#3A1A1A")  # red tint
            else:
...
def run_classifiers(X_train, X_test, y_train, y_test,
    ...
    results   = []
    fi_clfs   = {}   # name → fitted clf with feature_importances_
...
def main() -> None:
    ...
    print(f"[1/4] Extracting features from {args.pcap} …")
    try:
