...
We utilize the Attacker Node (`.74`) to intercept and discard all peer-to-peer communication to and from the Victim Node (`.71`), effectively partitioning it from the consensus network.
...
Retrieve the BootNode's enode URI and manually peer it with the Victim node.
```bash
# Get BootNode Enode
...
Upon reconnection, Node 162 will synchronize with its peers, evaluate the block heights, and recognize its local chain has a lower total difficulty. It will discard its isolated blocks (orphan the 5 ETH transaction) and adopt the canonical chain.

Run the balance check on Node 162 to confirm:
```bash
