...
* **The Main Network:** AS 161 (`.71` Miner) and AS 162 (`.71` Miner)
* **The Victim:** AS 160 (`as160h-Ethereum-POW-00-Miner-BootNode-10.160.0.71`)
* **The ARP Attacker (Layer 2):** AS 160 (`as160h-host_1-10.160.0.72`)
...
With the network partitioned, the blockchain splits into two competing states. As Node 160 continues to mine, it generates an isolated, independent chain. 
...
```bash
python3 fake_tx.py  # Sends 5 ETH to Victim Merchant (Isolated to Node 160)
python3 real_tx.py  # Sends 1 ETH to Attacker Safe (Broadcast to Nodes 161 & 162)
```

**2. Verify the State Divergence:**
Check the balances to confirm the network state has successfully fractured.

**Main Network Balances (Node 161 or 162):**
```bash
docker exec -it as161h-Ethereum-POW-01-Miner-10.161.0.71 geth attach --exec '["0xCBF1e330F0abD5c1ac979CF2B2B874cfD4902E24", "0xF5406927254d2dA7F7c28A61191e3Ff1f2400fe9", "0xaB5AaD8284868B91Eb537d28aB1A159740D54890"].forEach(addr => { console.log(addr + ": " + web3.fromWei(eth.getBalance(addr), "ether") + " ETH") })'
```
...
**Isolated Network Balances (Node 160):**
```bash
...
**1. Node 160 (BootNode Miner / The Victim)**
