...
if len(sys.argv) < 2:
    print ("Usage: blockchain-pow.py <option>")
...
eth = EthereumService()
blockchain = eth.createBlockchain(chainName="POW", consensus=ConsensusMechanism.POW)
...
for asn in asns:
    for id in range(hosts_total):
        ...
        if id == 0:
            ...
            emu.getVirtualNode(vnode).appendClassName("Miner")
        if i == 0:
            e.setBootNode(True)
...
newhost = emu.getLayer('Base').getAutonomousSystem(162).createHost('new_eth_node')
